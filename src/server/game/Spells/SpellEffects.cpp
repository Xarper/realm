                    ...
                    if (Player* player = m_caster->ToPlayer())
                    {
                        ...
                        if (uint32 combo = player->GetComboPoints())
                        {
                            ...
                            if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x00010000, 0, 0, m_caster->GetGUID()))
                            {
                                ...
                                damage = int32(player->GetTotalAttackPowerValue(BASE_ATTACK) * multi * 1 + (216 * doses));
...
//                    float average = ((m_caster->GetFloatValue(UNIT_FIELD_MINDAMAGE) + m_caster->GetFloatValue(UNIT_FIELD_MAXDAMAGE)) / 2) / m_caster->GetAttackTime(BASE_ATTACK);

                    //int32 count = m_caster->CalculateSpellDamage(unitTarget, m_spellInfo, EFFECT_2);
                    damage += 3.4 * (int32(average * IN_MILLISECONDS) / m_caster->GetAttackTime(BASE_ATTACK));
//                    damage = 4 * (average / m_caster->GetAttackTime(BASE_ATTACK));

//                    damage += 4 * int32(average * IN_MILLISECONDS) / GetAPMultiplier(attType,normalized);
                    break;
                }
